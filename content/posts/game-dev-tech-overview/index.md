...
最近突然对游戏开发技术产生了兴趣，那就写一篇技术文章聊一下这个东西。
...
它会按照你的想法，生成一个可执行的游戏程序。你负责：写逻辑，引擎负责：物理计算、屏幕渲染、音频播放……

总之，你只需要游戏的设计和编写，它负责按照你的想法用计算机语言构造出来。
...
该平台发布游戏最为简单：审核宽松，填写项目标题、描述，上传游戏文件，设置价格……

然后，就可以发布了。

无需其它成本和费用。

该平台与开发者的默认分钱比例为91分成，开发者也可以自行决定分成比例。

如果开发者想全拿，也是可以的。

### 2.2 steam

这个平台发布成本相对较高，除了填写必要的发布信息之外，还需要支付100美元的开发者费用，而且是每发布一个游戏，就需要支付一次。
...
该平台就是苹果商店，它的发布流程非常复杂，需要填写详细的发布信息，并等待审核（有可能被拒）。
...
如果你是个人开发者，那么发布一个微信小游戏，无需收费。

该平台审核较为严格，可能会需要你提供其它的额外的材料，例如：软著？

另外，如果游戏调用了后台服务器接口，那么需要对这个接口申请域名，并且备案。

### 2.5 邪修法

还有一种比较邪修的方式，就是部署在自己的云服务器上。
...
4、pc端 2D、3D 游戏的开发完全没有问题，steam上有很多独立游戏也都是用 unity 做的；
...
1、不太适合大型 3A 游戏，因为有个引擎更适合 3A 游戏，市场占有率更大，它就是 Unreal；

2、有一定的学习成本，需要掌握 C# 开发语言；
...
1、Unreal 引擎名气很大，有大厂背书。例如：黑神话悟空，堡垒之夜，都是用它开发的；
...
1、学习成本低，不需要任何费用，代码开源；

2、编程语言需要了解：GDScript，一看就知道这是一个脚本语言，简单易上手；

3、对于 2D 游戏，它的各种支持做得都很好；

劣势：

1、相对于其它主流引擎（Unity）来说，比较小众，社区虽然在蓬勃发展，但资源仍比较少；
...
3、适合微信小游戏、休闲手游、2D游戏、H5网页游戏的开发，代表作：开心消消乐、捕鱼达人，在这个赛道，它受众很广；

劣势：

1、3D能力偏弱，跟其它主流引擎相比差距明显；

2、社区圈子较小，国际影响力小，在海外存在感较低，主要是国内开发者在做贡献，所以，插件资源较少；

3、有一定的学习成本，需要了解TypeScript编程语言；

总的来说，这个引擎可以作为入门的选择。如果你是前端开发者，或者了解typescript编程语言，那么这个引擎就很适合你。
...
dota 是魔兽争霸的衍生品，它的影响力就不必多说了。

需要强调一点，mod 开发需要官方的支持。

官方提供了mod工具，玩家或者游戏创作者便可以进行“二创”。

如果官方没有提供mod的口子，而别有用心之人使用其它技术进行了二次修改，那就是严重的违规行为了。
